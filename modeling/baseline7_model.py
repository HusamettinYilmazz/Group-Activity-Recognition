...
class SequentialGroupActivityPooledPersons(nn.Module):
    def __init__(self, out_features):
        ...
        self.max_pool = nn.AdaptiveMaxPool2d((1, 2048))  ## from (12, 1024) to (1, 1024)
        self.layer_norm = nn.LayerNorm(2048)
        
        self.lstm2 = nn.LSTM(self.max_pool.output_size[1], hidden_size= 512, batch_first= True)
    ...
    def forward(self, x):   ## x.shape = [batch_size, seq_length, num_players, 3, 244, 244] 
        ...
        pooled_lstm1_out = self.max_pool(lstm1_out)
        pooled_lstm1_out = pooled_lstm1_out.view(batch_size, seq_length, -1)  ##[Batch_size, hidden_states = frames =9, 1024]
        
        pooled_lstm1_out = self.layer_norm(pooled_lstm1_out)        
        
        lstm2_out, (h2, c2) = self.lstm2(pooled_lstm1_out)
