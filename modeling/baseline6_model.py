...
class SequentialGroupActivityPooledPersons(nn.Module):
    ...
    def forward(self, x):   ## x.shape = [batch_size, seq_length, num_players, 3, 244, 244] 
        ...
        pooled_resnet_out = self.max_pool(resnet_out).view(batch_size, seq_length, -1)   ##[Batch_size, seq_length, 1024]

        lstm_out, (h, c) = self.lstm(pooled_resnet_out)  ## [Batch_size, 2048]
